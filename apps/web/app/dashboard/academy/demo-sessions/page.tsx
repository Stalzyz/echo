...
import { CalendarDays, Plus, MapPin, Users, Calendar, Loader2, PlayCircle, Video } from "lucide-react"
    ...
    <div className="flex flex-col h-full bg-slate-50 text-slate-900 p-8 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {(sessions || []).map((session: any) => (
          <div key={session.id} className="bg-white border border-slate-200 rounded-2xl p-6 hover:border-teal-400 transition-colors shadow-sm">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="text-[10px] px-2 py-1 rounded font-bold uppercase bg-emerald-50 text-emerald-700 border border-emerald-200">UPCOMING</span>
            </div>
      ...
      {isAddOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-md p-6 shadow-xl text-slate-900">
            <h2 className="text-xl font-bold mb-6 text-slate-900">Schedule Demo Class</h2>
            <form onSubmit={handleAdd} className="space-y-4">
              ...
              <div className="flex gap-3 pt-4 border-t border-slate-200">
                <button type="button" onClick={() => setIsAddOpen(false)} className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold transition-colors">Cancel</button>
