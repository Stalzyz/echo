...
export function registerGlobalListeners() {
  ...
  EventBus.on(SystemEvents.LEAD_CREATED, async (data) => {
    try {
      ...
      await EmailService.sendEmail(adminEmail, 'New Lead Arrived!', `<h1>New Lead</h1><p>${data.name} (${data.email}) just submitted an inquiry.</p>`).catch(() => {});
      ...
      console.error('[Autopilot] Error in LEAD_CREATED listener:', err);
    }
  });
  ...
  EventBus.on(SystemEvents.LEAD_CONTACTED, async (data) => {
    try {
      ...
      console.error('[Autopilot] Error in LEAD_CONTACTED listener:', err);
    }
  });
  ...
  EventBus.on(SystemEvents.PROPOSAL_SIGNED, async (data) => {
    try {
      ...
      await EmailService.sendEmail(data.clientEmail, 'Welcome to Grekam Visuals!', html).catch(() => {});
    } catch (err) {
      console.error('[Autopilot] Error in PROPOSAL_SIGNED listener:', err);
    }
  });
  ...
  EventBus.on(SystemEvents.INVOICE_PAID, async (data) => {
    try {
      ...
      await EmailService.sendEmail(data.clientEmail, 'Payment Receipt', html).catch(() => {});

      if (data.staffId) {
        await NotificationService.createNotification(data.staffId, 'Invoice Paid', `Invoice #${data.invoiceNumber} was just paid.`, `/dashboard/finance`, 'SUCCESS').catch(() => {});
      }

      if (data.clientPhone) {
        const numericAmount = typeof data.amount === 'number' ? data.amount : parseFloat(data.amount.replace(/[^0-9.]/g, '')) || 0;
        await whatsappService.sendInvoiceNotification(data.clientPhone, data.clientName, data.invoiceNumber, numericAmount).catch(() => {});
      }
    } catch (err) {
      console.error('[Autopilot] Error in INVOICE_PAID listener:', err);
    }
  });
  ...
  EventBus.on(SystemEvents.INVOICE_OVERDUE, async (data) => {
    try {
      ...
      console.error('[Autopilot] Error in INVOICE_OVERDUE listener:', err);
    }
  });
  ...
  EventBus.on(SystemEvents.INVOICE_CREATED, async (data) => {
    try {
      ...
      console.error('[Autopilot] Error in INVOICE_CREATED listener:', err);
    }
  });
  ...
  EventBus.on(SystemEvents.ASSIGNMENT_GRADED, async (data) => {
    try {
      ...
      await EmailService.sendEmail(data.studentEmail, 'Assignment Graded', html).catch(() => {});

      if (data.studentId) {
        await NotificationService.createNotification(data.studentId, 'Grade Updated', `Your assignment score is ${data.score}/100`, `/dashboard/lms/assignments`, 'INFO').catch(() => {});
        await GamificationService.evaluateAssignmentGrade(data.studentId, data.score, data.courseName).catch(() => {});
      }
    } catch (err) {
      console.error('[Autopilot] Error in ASSIGNMENT_GRADED listener:', err);
    }
  });
  ...
  EventBus.on(SystemEvents.ACADEMY_ENQUIRY_RECEIVED, async (data) => {
    try {
      ...
      await EmailService.sendEmail(data.email, `Welcome to Grekam Academy! Course Brochure`, html).catch(() => {});
      ...
      console.error('[Autopilot] Error in ACADEMY_ENQUIRY_RECEIVED listener:', err);
    }
  });
  ...
  EventBus.on(SystemEvents.ACADEMY_TRIAL_SCHEDULED, async (data) => {
    try {
      ...
      await EmailService.sendEmail(data.email, `Your Trial Class Details - Grekam Academy`, html).catch(() => {});
    } catch (err) {
      console.error('[Autopilot] Error in ACADEMY_TRIAL_SCHEDULED listener:', err);
    }
  });
  ...
  EventBus.on(SystemEvents.ACADEMY_LEAD_INACTIVE, async (data) => {
    try {
      ...
      await EmailService.sendEmail(data.email, `We miss you! Special Offer inside - Grekam Academy`, html).catch(() => {});
    } catch (err) {
      console.error('[Autopilot] Error in ACADEMY_LEAD_INACTIVE listener:', err);
    }
  });
  ...
  EventBus.on(SystemEvents.COURSE_COMPLETED, async (data) => {
    try {
      ...
      console.error('[Autopilot] Error in COURSE_COMPLETED listener:', err);
    }
  });
  ...
  EventBus.on(SystemEvents.FEE_PAID, async (data) => {
    try {
      ...
      if (data.studentEmail) {
        await EmailService.sendEmail(data.studentEmail, 'Fee Payment Confirmed - Grekam Academy', html).catch(() => {});
      }
      ...
      console.error('[Autopilot] Error in FEE_PAID listener:', err);
    }
  });
  ...
  EventBus.on(SystemEvents.FEE_DUE_REMINDER, async (data) => {
    try {
      ...
      if (data.studentEmail) {
        await EmailService.sendEmail(data.studentEmail, 'Fee Due Reminder - Grekam Academy', html).catch(() => {});
      }
      if (data.studentPhone) {
        await whatsappService.sendTemplateMessage({
          phone: data.studentPhone,
      ...
      console.error('[Autopilot] Error in FEE_DUE_REMINDER listener:', err);
    }
  });
  ...
  EventBus.on(SystemEvents.FEE_OVERDUE, async (data) => {
    try {
      ...
      if (data.studentEmail) {
        await EmailService.sendEmail(data.studentEmail, 'URGENT: Fee Overdue - Grekam Academy', html).catch(() => {});
      }
      if (data.studentPhone) {
        await whatsappService.sendTemplateMessage({
          phone: data.studentPhone,
      ...
      console.error('[Autopilot] Error in FEE_OVERDUE listener:', err);
    }
  });
  ...
  EventBus.on(SystemEvents.FEE_INVOICE_SENT, async (data) => {
    try {
      ...
      if (data.studentEmail) {
        await EmailService.sendEmail(data.studentEmail, `Fee Invoice: ${data.totalDue} due for ${data.batchName}`, html).catch(() => {});
      }
    } catch (err) {
      console.error('[Autopilot] Error in FEE_INVOICE_SENT listener:', err);
    }
  });
}
